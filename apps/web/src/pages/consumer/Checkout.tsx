...
import {
  addressSummary, addressTitle, buildAddress, defaultAddressIndex, validateAddress,
  type AddressObject, type CartBill, type SavedAddress,
} from '@marutham/lib';
...
export function Checkout({ bill, onOrderPlaced }: { bill: CartBill; onOrderPlaced: () => void }) {
  ...
  const [na, setNa] = useState<SavedAddress>({ label: 'Other', phone: (user?.phone as string) || '' });
  const [naError, setNaError] = useState<string | null>(null);

  const [pending, setPending] = useState<{ items: PlaceOrderItem[]; address: Record<string, unknown> | null } | null>(null);
  const [resolving, setResolving] = useState(false);

  function collectNewAddress(): Record<string, unknown> | null {
    const problem = validateAddress(na);
    if (problem) { setNaError(problem); toast(problem, 'er'); return null; }
    setNaError(null);
    return { ...na, label: na.label?.trim() || 'Other' } as Record<string, unknown>;
  }

  async function proceed() {
    if (cart.items.length === 0) { toast('Your cart is empty.', 'er'); return; }
    setResolving(true);
    try {
      ...
      for (const i of cart.items) {
        ...
        if (!farmerId) { toast('Some items have no available farmers. Please remove them.', 'er'); setResolving(false); return; }
      ...
      if (choice === 'new') {
        ...
        if (!address) { setResolving(false); return; }
  ...
  return (
    <div>
      <div className="fg">
        <label className="fl" htmlFor="checkout-addr">Delivery Address <span className="rq">*</span></label>
        <select id="checkout-addr" className="cons-select" style={{ padding: '10px 12px', fontSize: 13 }} value={choice} onChange={(e) => setChoice(e.target.value)}>
          {addrs.map((a, i) => <option key={i} value={String(i)}>{addrLine(a, i)}</option>)}
      ...
      <PaymentSheet
        ...
        onPlaced={() => { setPending(null); cart.clear(); onOrderPlaced(); }}
      />
    </div>
  );
}
