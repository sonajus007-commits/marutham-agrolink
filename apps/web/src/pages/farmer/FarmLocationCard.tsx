  ...
  async function capture() {
    ...
    try {
      ...
      try {
        ...
        updateUser(res.user);
        toast(t('farmer.loc.saved', 'Farm location saved.'), 'ok');
      } catch (e) {
        if (e instanceof OfflineQueuedError) {
          ...
          toast(
            t('farmer.loc.queued', 'Saved on your device — it will sync when you are back online.'),
            'ok',
          );
        } else {
          throw e;
      ...
      toast(
        e instanceof Error
          ? e.message
          : t('farmer.loc.saveFailed', 'Could not save farm location.'),
        'er',
      );
    } finally {
      setBusy(false);
  ...
  return (
    <Card>
      <h3 className="mb-1 text-md font-bold text-primary">
        📍 {t('farmer.loc.title', 'Farm location')}
      </h3>
      ...
      <div className="mb-3 rounded-sm bg-surface-muted p-3">
        <div className="text-2xs uppercase tracking-wide text-fg-muted">
          {t('farmer.loc.savedLabel', 'Saved location')}
        </div>
        <div className="text-sm font-semibold text-fg tabular-nums">
          {hasLoc
            ? `${lat!.toFixed(5)}, ${lng!.toFixed(5)}`
            : t('farmer.loc.notSet', 'Not set yet')}
        </div>
      </div>
      <Button onClick={capture} disabled={busy}>
        {busy
          ? t('farmer.loc.busy', 'Getting location…')
          : hasLoc
            ? `📍 ${t('farmer.loc.update', 'Update farm location')}`
            : `📍 ${t('farmer.loc.use', 'Use my current location')}`}
      </Button>
    </Card>
  );
}
