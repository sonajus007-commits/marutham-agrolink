...
export function ProductEditSheet({
  ...
  function addPriceRow() {
    ...
    setPrices((rows) =>
      [...rows, { district: addDistrict, market_price_rs: '', handling_rs: '0' }].sort((a, b) =>
  ...
  function setPrice(district: string, field: 'market_price_rs' | 'handling_rs', v: string) {
    setPrices((rows) =>
      rows.map((r) => (r.district === district ? { ...r, [field]: numeric(v) } : r)),
    );
  }
  ...
  async function save() {
    ...
    const priceInput: ProductPriceInput[] = prices
      ...
      .map((r) => ({
        district: r.district,
        market_price_rs: parseFloat(r.market_price_rs),
        handling_rs: parseFloat(r.handling_rs) || 0,
      }));
  ...
  return (
    <Sheet open={open} title={title} onClose={onClose}>
      {ro ? (
        <p className="mb-3 rounded-base bg-surface-muted px-3 py-2 text-2xs text-fg-muted">
          {t('admin.prod.readonly')}
        </p>
      ) : null}
      ...
      <Field label={t('admin.prod.field.regionalName')}>
        {(p) => (
          <Input
            ...
            onChange={(e) => set('regional_name', e.target.value)}
            disabled={ro}
            className="tamil"
          />
        )}
        ...
        <Field label={t('admin.prod.field.fee')}>
          {(p) => (
            <Input
              ...
              onChange={(e) => set('platform_fee_pct', numeric(e.target.value))}
              disabled={ro}
            />
          )}
        </Field>
      ...
      <div className="mb-3 flex flex-wrap gap-4">
        <label className="flex items-center gap-2 text-sm text-fg">
          <input
            type="checkbox"
            checked={form.exotic}
            onChange={(e) => set('exotic', e.target.checked)}
            disabled={ro}
          />
      ...
      <section className="mb-3 rounded-base border border-border-subtle bg-surface p-3">
        ...
        {prices.length === 0 ? (
          <p className="mb-2 text-2xs text-fg-muted">{t('admin.prod.noPrices')}</p>
        ) : null}
            ...
            <div key={r.district} className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-1">
                ...
                <input
                  ...
                  inputMode="decimal"
                  value={r.market_price_rs}
                  onChange={(e) => setPrice(r.district, 'market_price_rs', e.target.value)}
                  placeholder={t('admin.prod.market')}
                  disabled={ro}
                  aria-label={`${r.district} ${t('admin.prod.market')}`}
                />
              </div>
              {!ro ? (
                <button
                  type="button"
                  onClick={() => removePriceRow(r.district)}
                  aria-label={t('admin.prod.removeRow')}
        ...
        {!ro && availableDistricts.length > 0 ? (
          <div className="mt-2 flex items-center gap-2">
            <select
              ...
              aria-label={t('admin.prod.selectDistrict')}
            >
            ...
            <Button variant="ghost" onClick={addPriceRow} disabled={!addDistrict}>
              + {t('admin.prod.addDistrict')}
            </Button>
          </div>
        ) : null}
      </section>
      ...
      {!ro ? (
        <div className="flex flex-wrap items-center gap-2">
          <Button onClick={save} disabled={busy}>
            {busy ? t('admin.prod.saving') : t('admin.prod.save')}
          </Button>
