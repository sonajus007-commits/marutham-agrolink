...
export function OrdersPage() {
  ...
  const statusOptions = useMemo(() => {
    ...
    return [
      ...
      ...[...counts.entries()].sort((a, b) => b[1] - a[1]).map(([s, n]) => ({ value: s, label: `${s} (${n})` })),
    ];
  }, [orders, t]);
  ...
  const columns = useMemo<TableColumn<Order>[]>(() => [
    ...
    { key: 'district', header: t('admin.orders.district'), value: (o) => (o.district as string) || '' },
    {
      key: 'status',
  ...
  return (
    <>
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <Button variant="ghost" onClick={load} disabled={loading}>↻ {t('admin.orders.refresh')}</Button>
      </div>
      ...
      <AdminOrderSheet orderId={openId} open={openId !== null} onClose={() => setOpenId(null)} onChanged={load} />
    </>
  );
}
