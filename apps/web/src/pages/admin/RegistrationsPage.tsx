...
export function RegistrationsPage() {
  ...
  const statusOptions = useMemo(() => {
    ...
    regs.forEach((r) => { const s = statusOf(r); counts[s] = (counts[s] || 0) + 1; });
    return [
      { value: 'pending_review', label: `${t('admin.reg.status.pending_review')} (${counts.pending_review || 0})` },
      { value: 'payment_pending', label: `${t('admin.reg.status.payment_pending')} (${counts.payment_pending || 0})` },
  ...
  const columns = useMemo<TableColumn<Registration>[]>(() => [
    { key: 'login_id', header: t('admin.reg.loginId'), value: (r) => r.login_id || '', width: '150px' },
    { key: 'name', header: t('admin.reg.name'), value: (r) => `${r.fname || ''} ${r.lname || ''}`.trim() },
    { key: 'type', header: t('admin.reg.type'), value: (r) => (r.seller_type as string) || '' },
    { key: 'district', header: t('admin.reg.district'), value: (r) => (r.district as string) || '' },
    {
      key: 'status',
      ...
      render: (r) => {
        const s = statusOf(r);
        return (
          <span className="inline-block rounded-pill px-2 py-0.5 text-2xs font-bold text-white" style={{ background: REG_STATUS_TONE[s] || 'var(--fg-muted)' }}>
  ...
  return (
    <>
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <Button variant="ghost" onClick={load} disabled={loading}>↻ {t('admin.reg.refresh')}</Button>
      </div>
      ...
      <RegistrationDetailSheet regId={openId} open={openId !== null} onClose={() => setOpenId(null)} onChanged={load} />
    </>
  );
}
