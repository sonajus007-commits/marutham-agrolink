...
const roleLabel = (u: User) => (u.role === 'admin' ? String(u.admin_role || 'Admin') : String(u.role));
...
export function UsersPage() {
  ...
  const kindOptions = useMemo(() => {
    ...
    users.forEach((u) => { counts[kindOf(u)] = (counts[kindOf(u)] || 0) + 1; });
    return [
  ...
  const columns = useMemo<TableColumn<User>[]>(() => [
    { key: 'login_id', header: t('admin.users.loginId'), value: (u) => u.login_id || '', width: '150px' },
    { key: 'name', header: t('admin.users.name'), value: (u) => `${u.fname || ''} ${u.lname || ''}`.trim() },
    { key: 'role', header: t('admin.users.role'), value: (u) => roleLabel(u) },
    { key: 'district', header: t('admin.users.district'), value: (u) => (u.district as string) || '' },
    {
      key: 'status',
  ...
  return (
    <>
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <Button variant="ghost" onClick={load} disabled={loading}>↻ {t('admin.users.refresh')}</Button>
      </div>
      ...
      <UserDetailSheet userId={openId} open={openId !== null} onClose={() => setOpenId(null)} onChanged={load} />
    </>
  );
}
