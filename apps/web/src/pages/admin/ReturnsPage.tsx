...
export function ReturnsPage() {
  ...
  const statusOptions = useMemo(() => {
    ...
    returns.forEach((r) => { const s = returnStatus(r); counts[s] = (counts[s] || 0) + 1; });
    return [
      ...
      { value: 'collected', label: `${t('admin.ret.status.collected')} (${counts.collected || 0})` },
  ...
  const columns = useMemo<TableColumn<AdminReturn>[]>(() => [
    ...
    { key: 'customer', header: t('admin.ret.customer'), value: (r) => r.order?.consumer_name || '' },
    { key: 'type', header: t('admin.ret.type'), value: (r) => (r.full_return ? t('admin.ret.full') : t('admin.ret.partial')) },
    {
      key: 'refund',
    ...
    },
    {
      ...
      render: (r) => {
        ...
        return (
          <span className="inline-block rounded-pill px-2 py-0.5 text-2xs font-bold text-white" style={{ background: RETURN_STATUS_TONE[s] }}>
  ...
  return (
    <>
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <Button variant="ghost" onClick={load} disabled={loading}>↻ {t('admin.ret.refresh')}</Button>
      </div>
      ...
      <ReturnDetailSheet ret={open} open={open !== null} onClose={() => setOpen(null)} onChanged={load} />
    </>
  );
}
