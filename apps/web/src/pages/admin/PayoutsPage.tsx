...
import { Button, EmptyState, FilterChips, Modal, Spinner, StatTile, Table, type TableColumn } from '@marutham/ui';
...
const sumAmount = (list: AdminPayout[]) => list.reduce((s, p) => s + (parseFloat(String(p.amount)) || 0), 0);
...
export function PayoutsPage() {
  ...
  const columns = useMemo<TableColumn<AdminPayout>[]>(() => [
    { key: 'farmer', header: t('admin.pay.farmer'), value: (p) => farmerName(p) || (p.farmer?.phone || '') },
    ...
    },
    {
      ...
      render: (p) => (
        <span className="inline-block rounded-pill px-2 py-0.5 text-2xs font-bold text-white" style={{ background: PAYOUT_STATUS_TONE[String(p.status)] || 'var(--fg-muted)' }}>
    ...
    <>
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          {canSettle ? <Button onClick={() => setConfirmRun(true)} disabled={running}>⚡ {t('admin.pay.run')}</Button> : null}
          <Button variant="ghost" onClick={load} disabled={loading}>↻ {t('admin.pay.refresh')}</Button>
        </div>
      </div>

      <div className="mb-4 grid grid-cols-2 gap-3">
        <StatTile label={t('admin.pay.totalPaid')} value={fmtMoneyInt(sumAmount(paid))} hint={t('admin.pay.records', { count: paid.length })} accent="var(--success)" />
        <StatTile label={t('admin.pay.totalPending')} value={fmtMoneyInt(sumAmount(pending))} hint={t('admin.pay.records', { count: pending.length })} accent="var(--warning-strong)" />
      </div>
      ...
      <Modal
        ...
        footer={
          <>
            <Button variant="ghost" onClick={() => setConfirmRun(false)} disabled={running}>{t('admin.pay.cancel')}</Button>
            <Button onClick={runSettlement} disabled={running}>{running ? t('admin.pay.running') : t('admin.pay.run')}</Button>
          </>
        }
      >
